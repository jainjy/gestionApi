const associationsData = [
  ...
  {
    metierLibelle: "Electricien",
    serviceLibelle: "Installation d'une VMC (ventilation métaliique contrôlée)",
  },
  {
    metierLibelle: "Electricien",
  ...
  {
    metierLibelle: "Architecte Extérieur",
    serviceLibelle: "Terrasse en bois avec des charges réparties sur sol brut",
  },
  {
    metierLibelle: "Architecte Extérieur",
  ...
  {
    metierLibelle: "Diagnostiqueur",
    serviceLibelle: "Faire un devis pour des diagnostics d'un bien immobilier",
  },
  ...
  {
    metierLibelle: "Monteur en Installations de volet Persiennes et jalousies",
  ...
  {
    metierLibelle: "Association",
    serviceLibelle: "Récuperer des meubles, mobilier, appareils électroménager",
  },
  {
  ...
  {
    metierLibelle: "Monteur et Installateur de Grilles et Rideaux métalliques",
...
// Fonction pour supprimer les doublons basés sur metierLibelle et serviceLibelle
function supprimerDoublonsComplet(associationsData) {
  if (!Array.isArray(associationsData)) {
    throw new Error("Les données doivent être un tableau");
  }

  const associationsUniques = [];
  const dejaVus = new Set();

  for (const association of associationsData) {
    // Gestion robuste des valeurs nulles/undefined
    const metier = (association.metierLibelle || "")
      .toString()
      .trim()
      .toLowerCase();
    const service = (association.serviceLibelle || "")
      .toString()
      .trim()
      .toLowerCase();

    const cle = `${metier}|${service}`;

    if (!dejaVus.has(cle)) {
      dejaVus.add(cle);
      associationsUniques.push(association);
    }
  }

  return associationsUniques;
}

// Fonction pour afficher les statistiques
function afficherStatistiques(original, filtre) {
  console.log("📊 STATISTIQUES DES DOUBLONS");
  console.log("=".repeat(40));
  console.log(`Avant filtrage : ${original.length} associations`);
  console.log(`Après filtrage  : ${filtre.length} associations`);
  console.log(`Doublons supprimés : ${original.length - filtre.length}`);
  console.log(
    `Réduction : ${((1 - filtre.length / original.length) * 100).toFixed(1)}%`
  );
  console.log("=".repeat(40));
}

const associationsFiltrees = supprimerDoublonsComplet(associationsData);

// Afficher les résultats
afficherStatistiques(associationsData, associationsFiltrees);
...
// Sauvegarder en JSON
fs.writeFileSync(
  "associations_filtrees.json",
  JSON.stringify(associationsFiltrees, null, 2),
  "utf8"
);

console.log(
  '✅ Fichier "associations_filtrees.json" créé avec les données filtrées'
);
